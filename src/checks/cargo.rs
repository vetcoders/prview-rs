...
impl Check for CargoTestCheck {
    ...
    async fn run(&self, config: &Config) -> Result<CheckResult> {
        ...
        let mut run = plan_cargo_run(config)?;
        let started_clean_snapshot = matches!(
            super::resolve_scan_substrate(&run.cwd, &config.repo_root, &[]).tree_state,
            Some(super::TreeState::Snapshot)
        );
        if started_clean_snapshot {
            // Rust integration tests in the reviewed tree can invoke Python.
            // Keep its bytecode cache out of the immutable review substrate.
            run.env
        ...
        let snapshot_integrity_failure = if started_clean_snapshot {
            match super::snapshot_dirty_paths(cwd, &config.repo_root) {
                Some(paths) if paths.is_empty() => None,
                Some(paths) => Some(format!(
                    "prview snapshot integrity failure: cargo test modified the reviewed snapshot after starting clean; dirty paths: {}",
                    paths.join(", ")
                )),
                None => Some(
                    "prview snapshot integrity failure: cargo test finished but the reviewed snapshot status could not be verified"
                        .to_string(),
                ),
            }
        } else {
        ...
        if let Some(message) = &snapshot_integrity_failure {
            combined.push('\n');
            combined.push_str(message);
            combined.push('\n');
        }

        let status = if output.status.success() && snapshot_integrity_failure.is_none() {
            CheckStatus::Passed
        } else {
            CheckStatus::Failed
        };
...
mod tests {
    ...
    #[tokio::test]
    async fn cargo_test_fails_when_a_clean_snapshot_is_mutated() {
        ...
        std::fs::write(
            ...
            "[package]\nname = \"snapshot-mutator\"\nversion = \"0.0.0\"\nedition = \"2021\"\n",
        )
        .expect("fixture manifest");
        std::fs::create_dir_all(repo_root.path().join("src")).expect("fixture src");
        std::fs::write(
            repo_root.path().join("src/lib.rs"),
            r#"#[cfg(test)]
mod tests {
    #[test]
    fn mutates_the_reviewed_tree() {
        assert_eq!(
            std::env::var("PYTHONDONTWRITEBYTECODE").as_deref(),
            Ok("1"),
            "snapshot cargo tests must suppress Python bytecode writes",
        );
        std::fs::write(
            std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("generated.txt"),
            "generated by test\n",
        )
...
"#,
        ...
        let head = git2::Repository::open(repo_root.path())
            ...
            .id()
            .to_string();
        let snapshot = crate::git::create_worktree_snapshot(repo_root.path(), &head)
        ...
        config.scan_dir_override = Some(snapshot.worktree_path.clone());

        let result = CargoTestCheck.run(&config).await.expect("cargo test run");
        assert_eq!(
            result.provenance.as_ref().and_then(|p| p.exit_code),
            Some(0),
            "the fixture test itself passes; only the mutation oracle rejects the result",
        );
        assert_eq!(result.status, CheckStatus::Failed);
        assert!(
            result.output.contains("snapshot integrity failure"),
            "the failure must name the violated invariant: {}",
            result.output,
        );
        assert!(
            result.output.contains("generated.txt"),
            "the mutating path must remain auditable: {}",
            result.output,
        );
        assert_eq!(
            result.provenance.and_then(|p| p.tree_state),
            Some(super::super::TreeState::SnapshotDirty),
        );
    }
